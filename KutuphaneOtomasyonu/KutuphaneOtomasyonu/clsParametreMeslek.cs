...
namespace KutuphaneOtomasyonu
{
    class clsParametreMeslek
    {
        ...
        {
            string SQL = @"select * from tbl_meslek order by id";
        ...
        public void hazirid()
        {
            string SQL = @"select count(id) from tbl_meslek";
            ...
            {
                string sql = @"select max(id) from tbl_meslek";
                zDgr = da.ExecuteScaler(sql).ToString();
        ...
        public void sil()
        {
            ...
            string sql = @"select * from tbl_meslek where id =#id";
            sql = sql.Replace("#id", "'" + zId + "'");
            DataTable dt = new DataTable();
            dt = da.ExecuteDataTable(sql);
            if (dt.Rows.Count > 0)
            {
                string SQL = @"delete from tbl_meslek  where id = #id";
                SQL = SQL.Replace("#id", "'" + zId + "'");
                da.Execute(SQL);
                zDgr = "Silme Başarılı";
            }
        ...
        public void kaydet()
        {
            ...
            if (dt.Rows.Count == 0)
            {
                string SQL = @"insert into tbl_meslek values(#id,#tanim)";
                SQL = SQL.Replace("#id", "'" + zId + "'");
        ...
        public void guncelle()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                string SQL = @"update tbl_meslek set id=#id,tanim=#tanim where id=#id";
                SQL = SQL.Replace("#id", "'" + zId + "'");
                SQL = SQL.Replace("#tanim", "'" + zTanim + "'");
                da.Execute(SQL);
